...
from src.specialized_algorithms import aggregated_deletion_propagation
from src.gdp import gdp
from src.query import Query

CREATE_DATA = True
OVERWRITE_DATA = False
DATA_SAVE_FILE = 'data/expt_output/expt-data-adp-gdp-specialized-{}-k-{}.csv'
k = 50
k_in_percent = True
...
def runTestCase(case_no, query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10):
    ...
    """
    if CREATE_DATA:
        runMonotoneRunOfExperiments(query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = bag_semantics, max_bag_size = max_bag_size, output_filename=DATA_SAVE_FILE.format(case_no, k), overwrite_data = OVERWRITE_DATA)
        print('Experiment Data Generation complete. Data is stored in', DATA_SAVE_FILE.format(case_no, k))
    
    


def runMonotoneRunOfExperiments(query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10, output_filename='data/synthetic_expt/unknown_query_data.csv', overwrite_data = False, k_in_percent = True):
    """
    ...
    for itr_exponent in np.linspace(itr_log_start, itr_log_end, itr_num):
        ...
        output['number_of_witnesses'] = len(witnesses)
        output['k (percent)'] = k
        ...
        if adpk == 0:
            continue
        expt_results["lp_results"] = gdp(database_instance, query_del = [query], query_pres = None, kdel=[adpk], kpres=[], query_max = None, query_min = query_min, tuple_weights = tuple_weights, lp_type = "LP")
        if len(expt_results["lp_results"]) == 0:
            continue
        expt_results["ilp_results"] = gdp(database_instance, query_del = [query], query_pres = None, kdel=[adpk], kpres=[], query_max = None, query_min = query_min, tuple_weights = tuple_weights, lp_type = "ILP")
        if overtime_count < 3:
            expt_results["specialized_adp"] = aggregated_deletion_propagation(query, database_instance, adpk)
            if expt_results["specialized_adp"]["Solve Time"] >= timeout_time:
                overtime_count += 1
