...
DATA_SAVE_FILE = 'data/expt_output/expt-data-dpvs-gdp-specialized-{}.csv'
...
def runMonotoneRunOfExperiments(query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10, output_filename='data/synthetic_expt/unknown_query_data.csv', overwrite_data = False, k=10, k_in_percent = True):
    ...
    for itr_exponent in np.linspace(itr_log_start, itr_log_end, itr_num):
        ...
        expt_results = {}
        query_del = Query(query.name, query.head_vars, query.query_body, constants = tuple_to_delete)        
        expt_results["lp_results"] = gdp(database_instance, query_del = [query_del], query_pres = None, kdel=[1], kpres=[], query_max = None, query_min = query_min, tuple_weights = tuple_weights, lp_type = "LP")
        if len(expt_results["lp_results"]) == 0:
            continue
        expt_results["ilp_results"] = gdp(database_instance, query_del = [query_del], query_pres = None, kdel=[1], kpres=[], query_max = None, query_min = query_min, tuple_weights = tuple_weights, lp_type = "ILP")
        expt_results["specialized_dpvs"] = dp_view_side(query, database_instance, tuple_to_delete)
