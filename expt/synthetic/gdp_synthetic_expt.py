...
from src.utils import addTuples
...
def runMonotoneRunOfExperiments(query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10, output_filename='data/synthetic_expt/unknown_query_data.csv', overwrite_data = False):
    ...
    for itr_exponent in np.linspace(itr_log_start, itr_log_end, itr_num):
        ...
        # expt_results["lp_results"] = gdp(database_instance, tuple_weights = tuple_weights, lp_type = "LP")
        expt_results["lp_results"] = gdp(database_instance, query_del = [query], query_pres = [query], kdel=[1], kpres=[1], query_min = [query], tuple_weights = tuple_weights, lp_type = "LP")
        if len(expt_results["lp_results"]) == 0:
            continue
        expt_results["ilp_results"] = gdp(database_instance, query_del = [query], query_pres = [query], kdel=[1], kpres=[1], query_min = [query], tuple_weights = tuple_weights, lp_type = "ILP")
