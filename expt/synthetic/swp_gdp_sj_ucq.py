...
DATA_SAVE_FILE = 'data/expt_output/expt-data-swp-gdp-sj-ucq-{}.csv'
...
def runMonotoneRunOfExperiments(query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = True, max_bag_size = 10, output_filename='data/synthetic_expt/unknown_query_data.csv', overwrite_data = False):
    """
    ...
    if query_name == 'hc-sj-ucq':
        query_pres = [Query('R', ['x'], [('R', ['x', 'a','b']), ('R', ['x', 'b','c']), ('R', ['x', 'c', 'a'])]), 
                     Query('R', ['x'], [('R', ['x', 'e', 'f']), ('R', ['x', 'f', 'g'])])
                     ]
        query_max = [Query('R', ['x', 'a', 'b'], [('R', ['x', 'a', 'b'])])]
    else:
        query_pres = [query]
        query_max = [Query(table_name, table_vars, [(table_name, table_vars)] ) for (table_name, table_vars) in query.query_body]
    ...
    for itr_exponent in np.linspace(itr_log_start, itr_log_end, itr_num):
        ...
        kpres = []
        for q in query_pres:
            projections = witnesses[q][query.head_vars].drop_duplicates()
            kpres.append(len(projections))
            
        expt_results = {}
        
        expt_results["lp_results"] = gdp(database_instance, query_del = None, query_pres = [query], kdel=[], kpres=kpres, query_max = query_max, tuple_weights = tuple_weights, lp_type = "LP", treat_views_as_unions=True)
        if len(expt_results["lp_results"]) == 0:
            continue
        expt_results["ilp_results"] = gdp(database_instance, query_del = None, query_pres = [query], kdel=[], kpres=kpres, query_max = query_max, tuple_weights = tuple_weights, lp_type = "ILP", treat_views_as_unions=True)
