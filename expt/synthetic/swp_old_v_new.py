...
DATA_SAVE_FILE = 'data/expt_output/expt-data-case-swp-old-new-lp-{}.csv'
...
def runMonotoneRunOfExperiments(query_name, domain_size, itr_log_base, itr_log_start, itr_log_end, itr_num, bag_semantics = False, max_bag_size = 10, output_filename='data/synthetic_expt/unknown_query_data.csv', overwrite_data = False):
    ...
    query_max = [Query(table_name, table_vars, [(table_name, table_vars)] ) for (table_name, table_vars) in query.query_body]
    ...
    for itr_exponent in np.linspace(itr_log_start, itr_log_end, itr_num):
        ...
        expt_results["lp_results"] = gdp(database_instance, query_del = None, query_pres = [query], kdel=[], kpres=[len(projections)], query_max = query_max, tuple_weights = tuple_weights, lp_type = "LP")
        if len(expt_results["lp_results"]) == 0:
            continue
        expt_results["ilp_results"] = gdp(database_instance, query_del = None, query_pres = [query], kdel=[], kpres=[len(projections)], query_max = query_max, tuple_weights = tuple_weights, lp_type = "ILP")
        expt_results["ilp_results_old"] = gdp(database_instance, query_del = None, query_pres = [query], kdel=[], kpres=[len(projections)], query_max = query_max, tuple_weights = tuple_weights, lp_type = "ILP", old_ilp_formulation = True)
        expt_results["lp_results_old"] = gdp(database_instance, query_del = None, query_pres = [query], kdel=[], kpres=[len(projections)], query_max = query_max, tuple_weights = tuple_weights, lp_type = "LP", old_ilp_formulation = True, verbosity=0)
